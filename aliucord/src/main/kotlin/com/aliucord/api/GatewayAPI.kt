...
public object GatewayAPI {
    ...
    private fun patchRawMessageHandler() {
        patcher.after<`GatewaySocket$connect$$inlined$apply$lambda$4`>("onRawMessage", String::class.java) { (_, rawEvent: String) ->
            if (eventListeners.isNotEmpty()) {
                ...
                eventListeners
                    .filter { (name, _) -> name == eventName }
                    .forEach { (_, type, listener) ->
                        try {
            ...
            if (rawListeners.isNotEmpty()) {
                rawListeners.forEach { listener -> listener.invoke(rawEvent) }
            }
        }
    }
     ...
     */
    @JvmStatic
    public fun onRawEvent(listener: EventListener<String>): Boolean = rawListeners.add(listener)
     ...
     */
    @JvmStatic
    public fun onRawEvent(name: String, listener: (rawEvent: String) -> Unit): Unit = onRawEvent(listOf(name), listener)
     ...
     */
    public inline fun <reified T : Any> onEvent(name: String, crossinline listener: (T) -> Unit) {
        val eventListener = EventListener<Any> { eventData -> listener(eventData as T) }
        eventListeners += Triple(name.uppercase(), T::class.java, eventListener)
    }
    ...
    public fun <T> onEvent(name: String, clazz: Class<T>, listener: (T) -> Unit) {
        val eventListener = EventListener<Any> { eventData -> listener(eventData as T) }
        eventListeners += Triple(name.uppercase(), clazz, eventListener)
    }
}
