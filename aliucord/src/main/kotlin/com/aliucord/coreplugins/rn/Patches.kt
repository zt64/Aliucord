...
internal class RNHeadersInterceptor : Interceptor {
    override fun intercept(chain: Interceptor.Chain): Response {
        val req = chain.c()
        val headersBuilder = req.d.e().apply {
            d("X-Super-Properties")
            a("X-Super-Properties", RNSuperProperties.superPropertiesBase64)
            d("User-Agent")
            a("User-Agent", RNSuperProperties.USER_AGENT)
            a("X-Discord-Timezone", TimeZone.getDefault().id)
        }
...
internal fun patchNextCallAdapter() {
    ...
    GlobalPatcher.addPatch(i0.y::class.java.getDeclaredMethod("a", Type::class.java, Array<Annotation>::class.java), PreHook {
        when (it.args[0]) {
...
private val globalNames = mutableMapOf<Long, String>()
internal fun patchUser() {
    val original = User::class.java
    val new = RNUser::class.java

    GlobalPatcher.addPatch(InboundGatewayGsonParser::class.java.getDeclaredMethod("fromJson", JsonReader::class.java, Class::class.java), PreHook {
        if (it.args[1] == original) it.args[1] = new
    })

    GlobalPatcher.addPatch(UserUtils::class.java.getDeclaredMethod("padDiscriminator", Int::class.java), PreHook {
    ...
    GlobalPatcher.addPatch(CoreUser::class.java.getDeclaredConstructor(User::class.java), hook)
    GlobalPatcher.addPatch(MeUser::class.java.getDeclaredConstructor(User::class.java), hook)

    GlobalPatcher.addPatch(
        GuildMember.Companion::class.java.getDeclaredMethod(
            ...
            if (it.result == user.username && user.id in globalNames) it.result = globalNames[user.id]
        })

    GlobalPatcher.addPatch(
        UserNameFormatterKt::class.java.getDeclaredMethod(
    ...
    GlobalPatcher.addPatch(`ChannelUtils$getDisplayName$1`::class.java.getDeclaredMethod("invoke", Any::class.java), PreHook {
        val user = it.args[0]
...
internal fun patchUserProfile() {
    val interceptor = RNHeadersInterceptor()

    GlobalPatcher.addPatch(f0.e0.h.g::class.java.declaredConstructors[0], PreHook {
    ...
    GlobalPatcher.after<UserProfile> {
