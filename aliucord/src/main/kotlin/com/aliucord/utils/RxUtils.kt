...
public object RxUtils {
     ...
     */
    @JvmStatic
    public fun <T> Observable<T>.onBackpressureBuffer(): Observable<T> = K()
     ...
     */
    @JvmStatic
    public fun <T> Observable<T>.subscribe(subscriber: Subscriber<in T>): Subscription = this.U(subscriber)
     ...
     */
    @JvmStatic
    public fun <T> Observable<T>.subscribe(onNext: T.() -> Unit): Subscription = subscribe(createActionSubscriber(onNext))
    ...
    public fun <T> Observable<T>.await(): Pair<T?, Throwable?> {
    ...
    @JvmStatic
    @JvmOverloads
    public fun <T> createActionSubscriber(
        onNext: Action1<in T>,
     ...
     */
    @JvmStatic
    public fun timer(delay: Long, unit: TimeUnit?): Observable<Long> = Observable.e0(delay, unit, j0.p.a.a())
     ...
     */
    @JvmStatic
    public fun <T, R> combineLatest(sources: List<Observable<T>>, combineFunction: FuncN<R>): Observable<R> {
        return Observable.b(sources, combineFunction)
    }
     ...
     */
    @JvmStatic
    public fun <T, R> Observable<T>.map(func: (T) -> R): Observable<R> = G { func(it) }
     ...
     */
    @JvmStatic
    public fun <T, R> Observable<T>.switchMap(func: (T) -> Observable<R>): Observable<R> = Y { func(it) }

    @JvmStatic
    public val empty: Observable<Any> = c.k
}
