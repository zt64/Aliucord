...
abstract class PatchManifestTask : Copy() {
    @get:InputFile
    abstract val manifest: RegularFileProperty

    @get:OutputFile
    abstract val patchedManifest: RegularFileProperty

    @TaskAction
    fun patchManifest() {
        manifest.get().asFile.copyTo(patchedManifest.get().asFile)
    }
}
                    ...
                    override fun child(ns: String?, name: String): NodeVisitor {
                        ...
                        if (addExternalStoragePerm) {
                            super
                                ...
                                .attr(ANDROID_NAMESPACE, "name", ATTR_NAME, TYPE_STRING, MANAGE_EXTERNAL_STORAGE)
                        ...
                        return when (name) {
                            "uses-permission" -> object : NodeVisitor(nv) {
                                override fun attr(ns: String?, name: String?, resourceId: Int, type: Int, value: Any?) {
                                    ...
                                    if (name == "name" && value == READ_EXTERNAL_STORAGE) {
                            ...
                            "uses-sdk" -> object : NodeVisitor(nv) {
                                override fun attr(ns: String?, name: String?, resourceId: Int, type: Int, value: Any?) {
                                    if (name == "targetSdkVersion") {
                                        ...
                                        super.attr(ns, name, resourceId, type, 30)
                                    } else {
                                        super.attr(ns, name, resourceId, type, value)
                                    }
                                }
